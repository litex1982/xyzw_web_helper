            ...
            >
              <div class="lineup-title-bar" @click="toggleLineupExpand(lineup)">
                ...
                <div class="lineup-quick-actions">
                  <n-button size="tiny" @click.stop="renameLineup(savedLineups.indexOf(lineup))">
                    重命名
                  </n-button>
                  <n-button
                    size="tiny"
...
const getHeroMapping = async () => {
  ...
  try {
        ...
        const heros = roleRes?.role?.heroes
    return heros;
  } catch (e) {
    console.error("获取英雄映射失败:", e);
    return {};
  }
};

const getPearlMapping = async () => {
    if (!tokenStore.selectedToken) {
    message.warning("请先选择Token");
    return;
  }
  try {
        const tokenId = tokenStore.selectedToken.id;
        const roleRes = await tokenStore.sendMessageWithPromise(tokenId, 'role_getroleinfo', {}, 10000)
        const heros = roleRes?.role?.pearlMap
    return heros;
  } catch (e) {
    console.error("获取英雄映射失败:", e);
    return {};
...
const getPearlSkill = (pearlMap, pearlId) => {
  return Object.values(pearlMap).find((pearl) => pearl.pearlId === pearlId)?.skillId;
};

const getHeroIdByAttachmentUid = (heroesList, attachmentUid) => {
  return Object.values(heroesList).find((hero) => hero.attachmentUid === attachmentUid)?.heroId;
};
  ...
  try {
    ...
    const heroesData = editingHeroes.value.map((hero) => {
      return {
        ...
        pearlId: hero.pearlId,
        skillId: getPearlSkill(pearlMap, hero.pearlId)
      };
    });
    ...
    savedLineups.value.unshift({
      ...
      weaponId: weaponId,
      pearlMap: pearlMap,
      heroMap: Object.values(heroesList).filter((hero) => {
            //鱼珠只能通过鱼灵挂载，所以过滤鱼灵artifactId
            return hero.artifactId > 0;
        }).map((hero) => ({ attachmentUid: hero.attachmentUid, heroId: hero.heroId, artifactId: hero.artifactId, pearlId: hero.pearlId, battleTeamSlot: hero.battleTeamSlot })),
    });
...
const applyLineup = async (lineup) => {
  ...
  if (lineup.teamId !== currentTeamId.value) {
    ...
    message.warning(`当前阵容与要加载的阵容ID不一致，正在切换到阵容 ${lineup.teamId}...`);
    await switchTeam(lineup.teamId);
  }
...
const applyFishSetting = async (localTeamSettings) => {
  try {
    ...
    //恢复所有鱼珠的技能
    const pearlsToload  = targetPearlMap.filter((pearl) => {
      return pearl.skillId > 0;
    }).map((pearl) => ({ pearlId: pearl.pearlId, skillId: pearl.skillId }));
    ...
    //卸载所有鱼灵
    for await (const heroToUnload of Object.values(heroesList).filter((hero) => {
        return hero.artifactId > 0;
    ...
    //根据attachmentUid恢复所有鱼灵装置
    for await (const heroToLoad of teamToLoad.heroMap.map((hero) => (
        { heroId: hero.heroId, tempHeroId: getHeroIdByAttachmentUid(heroesList, hero.attachmentUid), artifactId: hero.artifactId, attachmentUid: hero.attachmentUid, pearlId: hero.pearlId }
        )
      )
  ) {
